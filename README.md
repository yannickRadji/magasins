...
Here is the lineage of the projects tables generated by DBT ![alt text](img/imagedbt.png)

With the commands "dbt docs generate", then "dbt docs serve" you can see the definition and how the tables has been created, for example below the magasins_match & dim_magasins.
![alt text](img/docgold.png)
![alt text](img/docsilver.png)
...
## CICD
This flyway CICD use clean to ensure that correct Snowflake objects are delivered at each run even if the previous has failed but in real world we should handle the case of failure with repair/undo and not use clean. flyway_schema_history should be set to be in the AUDIT schema. The documentation generate by DBT is a website so it can be hosted on an Azure Web App.

We should be able to deploy application version based on git tag to more easily communicate which version is deployed or roll back. As we are now on a public repo I couldn't enforced rulesets and all protections.
...
We have data quality KPIs (that could be enhance via more advanced rules), visualizations could be embeeded in Snowflake with streamlit but because of time constraint i did it in powerBI.
